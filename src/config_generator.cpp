...
int main(int argc, char** argv) {
	...
	if (!writeJson(outputDir / "config.json", headfetch::makeDefaultConfigJson())) return 1;
	if (!writeJson(outputDir / "config.schema.json", headfetch::makeConfigSchemaJson())) return 1;
